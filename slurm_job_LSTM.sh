...
# #SBATCH --gres=gpu:2 #most machines have a single GPU, so leave this as-is. If you are on a dual GPU partition, this can be changed to --gres=gpu:2 to use both
#SBATCH --mem-per-cpu=70G         # (12G) (30G) (72G) memory per cpu-core - unless you're doing something obscene the default here is fine. This is RAM, not VRAM, so it's like storage for your dataset
