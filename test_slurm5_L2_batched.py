...
look_back_list = list(range(5000, 6001, 1000))
...
# headroom_list = np.arange(1.0, 0.1, -0.1)

headroom_list = [0.1]

total_param_comb = len(look_back_list) * len(batch_size_list) * len(epoch_list)
...
def forecast(model, start_input, steps, look_back, batch_size):
	...
	forecast_model.add(LSTM(neurons, activation='tanh', return_sequences=True))
		...
		for bs in batch_size_list:
			for ep in epoch_list:
				...
				for curr_col, curr_system in enumerate(system_names):
					...
					model.add(LSTM(neurons, activation='tanh', stateful=True, return_sequences=True))
